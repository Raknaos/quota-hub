...
function renderKeys(){ const box=$('keys-login-box'), table=$('keys-table-container'); if(!box||!table)return; const logged=!!APP_STATE.me; box.style.display=logged?'none':'block'; table.style.display=logged?'block':'none'; if(!logged)return; const keys=APP_STATE.keys||[]; $('keys-table-body').innerHTML=keys.length?keys.map(k=>`<tr><td><strong>${escapeHtml(k.name)}</strong></td><td><code>${escapeHtml(k.prefix)}…</code></td><td>${new Date((k.created_at||0)*1000).toISOString().slice(0,10)}</td><td>${Number(k.n||0).toLocaleString('fr-FR')} req · ${Number(k.tok||0).toLocaleString('fr-FR')} tok</td><td><span class="key-state ${k.revoked?'revoked':''}">${k.revoked?'Révoquée':'Active'}</span></td><td>${k.revoked?'':`<button class="table-action" onclick="revokeKey(${k.id})">Révoquer</button>`}</td></tr>`).join(''):`<tr><td colspan="6" class="empty-table">Aucune clé. Créez votre première clé.</td></tr>`; }
...
async function loadUsage(){
  ...
  if(r.status!==200){ $('usage-table-body').innerHTML='<tr><td colspan="7" class="empty-table">'+escapeHtml(r.data.error?.message||'Journaux indisponibles')+'</td></tr>'; return; }
  ...
  const rows=d.rows||[];
  $('usage-table-body').innerHTML = rows.length ? rows.map(x=>`<tr><td>${new Date((x.ts||0)*1000).toLocaleString('fr-FR',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}</td><td><code>${escapeHtml(x.key||'—')}</code></td><td><code>${escapeHtml(x.model)}</code></td><td>${nf(x.in)}</td><td>${nf(x.out)}</td><td><b>${nf(x.tokens)}</b></td><td>$${Number(x.cost_usd||0).toFixed(5)}</td></tr>`).join('') : '<tr><td colspan="7" class="empty-table">Aucune requête pour le moment — lancez-en une depuis le playground ou avec votre clé.</td></tr>';
  ...
  const krows=d.per_key||[];
  $('usage-keys').innerHTML = krows.length ? krows.map((it,i)=>`<div class="usage-model-item"><i style="background:${COLORS[i%COLORS.length]}"></i><span>${escapeHtml(it.key)}</span><b>${nf(it.tokens)} tok · ${nf(it.n)} req</b></div>`).join('') : '<span class="auto-mix-hint">Aucune consommation par clé sur les 30 derniers jours.</span>';
