...
# Pool AUTO principal (14-09) — 4 modèles, TOUS MULTIMODAUX.
# VÉRIFIÉ PAR TEST RÉEL (14-09, image de contrôle 4 quadrants à ordre non
# devinable) : qwen3.8-flash, glm-5.3-flash, deepseek-v4.1-flash et grok-4.6
# ont TOUS lu les 4 couleurs correctement. La vision n'est donc pas un cas
# particulier : plus de pool vision, les images routent comme le texte.
# Retirés : 'deepseek-v4-flash-vision' (inexistant côté amont -> 400 « 请求参数
# 不正确 ») et 'deepseek-v4-flash-vision-exp' (canal à sec « 可用额度不足 »,
# et redondant puisque tout le pool voit).
...
POOL_VELOCE = ['qwen3.8-flash', 'glm-5.3-flash', 'deepseek-v4.1-flash']  # 14-09 : deepseek-v4-flash (mort) retiré
...
def market30_public(d30=None):
    """Vue marché PUBLIQUE (14-09) : mêmes chiffres que /api/prices mais SANS
    l'identité de la chaîne d'approvisionnement — aucun nom de revendeur, aucun
    id de canal, aucun tarif des canaux non élus. Ferme la fuite concurrentielle
    (un concurrent obtenait la liste des fournisseurs et le prix d'achat)."""
    d = d30 if isinstance(d30, dict) else (market30_all() or {})
    out = {'ok': bool(d), 'age_s': d.get('age_s'), 'max_age_s': d.get('max_age_s'),
           'generated_at': d.get('generated_at'), 'models': {}}
    for mid, ent in (d.get('models') or {}).items():
        tops = ent.get('top') or []
        pub = []
        for t in tops[:1]:
...
def parse_upstream_response(raw_bytes, requested_model, payload=None):
        ...
        try:
            ...
            if isinstance(data, dict) and 'choices' in data and data['choices']:
                ...
                if isinstance(c0, dict):
                    ...
                    # 14-09 : on NE RÉÉCRIT PLUS finish_reason. Transformer
                    # 'length' en 'stop' cachait au client une réponse TRONQUÉE
                    # (il croyait la génération finie). L'honnêteté prime.
    ...
    # 14-09 : plus de réécriture 'length' -> 'stop' (troncature visible).
...
def chat_auto(payload, is_stream, plan='auto', key_id=None):
    ...
    Plan 'auto'   : famille flash polyvalente (qwen/glm/deepseek v4/grok), TOUS
                    multimodaux : les images routent sur le pool normal.
    ...
    if mt is None:
        ...
        if mt < 2048:
            ...
            body['max_tokens'] = MAX_TOKENS_CAP
    # 14-09 : PLUS DE POOL VISION. Les 4 modèles du pool lisent les images
    # (mesuré, pas déduit du nom du modèle). Les requêtes avec images suivent
    # donc exactement le même routage que le texte : « moins cher vivant ».
    pool = list(MODELS)
...
def chat_auto_stream(payload, plan, key_id, uid):
    ...
    if mt is None:
        ...
        if mt < 2048:
            ...
            body['max_tokens'] = MAX_TOKENS_CAP
    # 14-09 : plus de pool vision ici non plus — les images routent sur le pool normal.
    ...
    for attempt in range(3):
        ...
        if not ok:
            ...
            continue
        b = chosen or ((market_get(model_id) or {}).get('best') or {})
        _fc = round(time.time() - t_start, 3)      # latence RÉELLE du 1er jeton
        log(f'STREAM {model_id} via {b.get("supplier")} 1er chunk {_fc}s')
        _pin_set(ctx, model_id, b.get('supplier'))
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _relay_sse(self, rl, headers=None):
        ...
        b = rl.get('best') or {}
        if not u:
            # HONNÊTETÉ CACHE (14-09) : estimer sans décote de cache SURFACTURE
            # le client (il paierait plein tarif des tokens servis du cache
            # amont). On applique le taux de cache réellement mesuré du canal.
            try:
                _rr = real_cache_rate(rl['model_id'], b.get('supplier'))
                if _rr and _rr > 0:
                    cached_tok = int(tin * min(0.95, float(_rr)))
            except Exception:
                pass
        ...
        try:
            billed, remaining = bill_and_log(rl['uid'], rl['plan'], rl['key_id'],
                                             ...
                                             int(float(rl.get('first_chunk_s') or 0) * 1000))
    ...
    def do_GET(self):
        ...
        if path == '/health':
            ...
            return
        if path == '/api/prices':
            # 14-09 : cette vue expose la chaîne d'approvisionnement (noms des
            # revendeurs, prix d'achat, ids de canal). Elle n'est PLUS servie en
            # clair : signature HMAC du front Vercel exigée hors loopback, et le
            # client reçoit une vue ANONYMISÉE (aucun nom de fournisseur).
            _lb = self.client_address[0] in ('127.0.0.1', '::1')
            if not _lb and not check_sig(self.headers, path, b''):
                return self._json(403, {'error': {'message': 'signature invalide'}})
            d30 = market30_all() or {}
            if not d30:
                return self._json(200, {'ok': False, 'models': {}})
            return self._json(200, d30 if _lb else market30_public(d30))
        if path == '/api/public/stats':
            try:
                ...
                # 14-09 : latence p50 du 1er jeton MESURÉE (24 h) + part réelle de
                # cache, pour que le site n'affiche plus de chiffres inventés.
                t24 = int(time.time()) - 86400
                lats = [r[0] for r in c.execute(
                    'SELECT latency_ms FROM usage_logs WHERE created_at >= ? AND latency_ms > 0 '
                    'ORDER BY latency_ms', (t24,)).fetchall()]
                p50 = lats[len(lats) // 2] if lats else None
                tot_tok = row[1] if row else 0
                return self._json(200, {
                    ...
                    'p50_first_token_ms_24h': p50,
                    'latency_samples_24h': len(lats),
                    'cache_share': round((row[2] / tot_tok) if tot_tok else 0, 4),
                    'ts': time.time()
                })
            except Exception as e:
